...
| **`fortress-truth`** | The cleared/uncleared claim register. Installs and runs `claim_lint.py`, which fails on unsourced claim-shaped language: bare numbers/percentages, superlatives, comparatives, absolutes, testimonial-shaped quotes. Bans the motte-and-bailey rhetorical move outright. No per-finding waiver: detection patterns are fixed and not user-overridable, severities are constrained, and every `ignore` pattern in effect is printed on every run. |
